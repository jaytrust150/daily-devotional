...
function displayDevotional() {
    ...
    const filename = `${formattedDate}-devotional.txt`;
    fetch(filename)
        .then(response => {
            if (!response.ok) {
...
shareButtons.forEach(button => {
  button.addEventListener('click', () => {
    // Get the content to share (devotional or podcast) relative to the clicked button
    const devotionalTextDiv = button.closest('nav').querySelector('#devotionalText'); 
    const contentToShare = devotionalTextDiv.textContent;

    // Show the share options (relative to the clicked button)
    const shareOptions = button.closest('nav').querySelector('.share-options');
    shareOptions.style.display = shareOptions.style.display === 'none' ? 'block' : 'none';

    // Share via Facebook
    const shareFacebook = shareOptions.querySelector('.share-facebook');
    ...
    shareFacebook.href = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent('https://jaytrust150.github.io/daily-devotional/')}`;
    shareFacebook.target = '_blank';

    // Share via Instagram
    const shareInstagram = shareOptions.querySelector('.share-instagram');
    shareInstagram.href = `https://www.instagram.com/`;
    shareInstagram.target = '_blank';

    // Share via Snapchat
    const shareSnapchat = shareOptions.querySelector('.share-snapchat');
    shareSnapchat.href = `https://www.snapchat.com/`;
    shareSnapchat.target = '_blank';

    // Share via Email
    const shareEmail = shareOptions.querySelector('.share-email');
    const emailBody = `${contentToShare.trim()}\n\nRead more at: [your website link here]`; // Replace [your website link here] with the actual link
    shareEmail.href = `mailto:?subject=Shared Devotional&body=${encodeURIComponent(emailBody)}`;

    // Share via Twitter (X)
    const shareX = shareOptions.querySelector('.share-x');
    shareX.href = `https://twitter.com/intent/tweet?text=${encodeURIComponent(contentToShare.trim())}`;
    shareX.target = '_blank';

    // Share via Reddit
    const shareReddit = shareOptions.querySelector('.share-reddit');
    shareReddit.href = `https://www.reddit.com/submit?title=Shared%20Devotional&text=${encodeURIComponent(contentToShare.trim())}`;
    shareReddit.target = '_blank';

    // Share via Google Docs
    const shareGoogleDocs = shareOptions.querySelector('.share-google-docs');
    ...
    // Share via Text (SMS)
    const shareText = shareOptions.querySelector('.share-text');
...
const prevDevotionalButtons = document.querySelectorAll('#prevDevotional');
const nextDayButtons = document.querySelectorAll('#nextDevotional');

prevDevotionalButtons.forEach(button => {
    button.addEventListener('click', prevDay);
});
